...
//  g++ segway_curves.cpp -o motion -I/usr/include/python3.8 -lpython3.8
...
std::tuple<std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>, std::vector<float>> methodRuneKuttaSegway()
{
    ...
    // init values
    float x1 = (M_PI / 180) * 50; // theta - pendulum
...
void plot2D2D(std::tuple<std::vector<float>, std::vector<float>> data1, std::tuple<std::vector<float>, std::vector<float>> data2)
{
    ...
    plt::title("Segway robot motion");
    plt::named_plot("robot pos", xX1, yY1);
    plt::named_plot("motor pos", xX2, yY2);
    plt::xlabel("time");
    plt::ylabel("[rad]");
    plt::legend();
    plt::show();
